...
class Core:
    ...
    def download_project(self, hash_id):
        ...
        for asset in assets:
            ...
            if not self.no_image and asset['has_image']:  # 包含图片
                ...
                try:
                    self.futures.append(self.executor.submit(self.download_file, url, file_path, file_name))
            ...
            if not self.no_video and asset['has_embedded_player']:  # 包含视频
                ...
                try:
                    self.futures.append(self.executor_video.submit(self.download_video, id, file_path))
                except Exception as e:
                    print(e)
    ...
    def download_by_username(self, username):
        ...
        if len(data) is not 0:
            future_list = []
            for project in data:
                future = self.executor.submit(self.download_project, project['hash_id'])
